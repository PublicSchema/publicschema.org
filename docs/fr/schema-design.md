...
Les deux paires génériques ne sont pas des alias. `start_date` nomme le premier jour où une relation est effective et `end_date` le dernier, tous deux inclus ([ADR-027](../../decisions/027-end-date-boundary.md)). `valid_from` et `valid_to` nomment le premier et le dernier jour calendaire applicables de la validité d'une assertion. `recorded_at` enregistre plutôt le moment où la source a saisi l'assertion. Les dates absentes restent inconnues ; une fin omise ne prouve pas une validité perpétuelle.

Utilisez `start_date` / `end_date` pour les concepts de relation et d'appartenance, tels que HoldingParcelLink, AnimalResidence, AnimalResponsibility, AgriculturalServiceRole, IdentifierAssignment, NameUsage, ContactPoint, AssetPartyRole et AssetAddressAssignment. Registration (y compris les spécialisations d'Authorization telles que DrivingEntitlement), RegistryEntry, AgriculturalParcel, Certification et LandTenureAssertion conservent leur validité calendaire déclarée. Le [guide de conversion des dates de relation](../relationship-date-migration.md) décrit le contrat de conversion explicite pour les enregistrements sources qui utilisent une validité calendaire.

Une application qui compare ou convertit des dates doit d'abord connaître les frontières des intervalles de la source. Les deux paires incluent leur dernier jour : un `valid_to: 2026-06-30` inclusif de la source devient donc `end_date: 2026-06-30`, et la période suivante commence le 1er juillet. Une source dont la fin est le premier jour qui ne s'applique plus exige de retrancher un jour. Ne convertissez pas lorsque la précision de la source ou sa sémantique de frontière est inconnue. L'exemple de travail agricole suit la même convention.
