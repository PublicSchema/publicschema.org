...
A source that has concluded whether an interest amounts to beneficial ownership or control records it in `beneficial_ownership_or_control`, which corresponds to the BODS `beneficialOwnershipOrControl` field, together with `beneficial_ownership_definition`, the URI of the legal definition it applied. The vocabulary records that conclusion; it does not reach one. The example profile rejects the conclusion without its definition. An absent value means no conclusion is recorded, not that the interest does not qualify.
...
`qualification_awarded` identifies the qualification definitions, such as entries in a qualifications register, that a program or offering leads to. It is distinct from AwardedQualification, which describes an award already made to a person. An AwardedQualification names the same kind of definition URI in `qualification_definition`, so an award can be matched to the programs that lead to it. The example retains the definition URI without fetching it or verifying its external meaning. It does not introduce another qualification catalog or infer that any student completed the program.
