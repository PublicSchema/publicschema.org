...
PublicSchema `Organization` and `health/HealthFacility` provide independently useful cross-sector identities in the sidecar. Their medical detail stays in native FHIR. HealthFacility denotes physical premises and is not equivalent to every FHIR Location. This example requires its bound Location to have `mode: instance` and an explicit site/building classification (`si`/`bu` in `http://terminology.hl7.org/CodeSystem/location-physical-type`); conceptual and virtual locations are rejected.

Medicinal and substance records omit the optional PublicSchema `subject_type`: the example does not mint a replacement native medical class or use a FHIR resource class as proof of a real-world subject's type. Instead, every consumer link declares `expected_resource_type`, which is checked against the resolved native record. The returned value includes the unchanged FHIR resource and the separately asserted `subject_uri`.
...
## Mapping flat health registry records to FHIR

PublicSchema has no native medicinal product, healthcare service or accreditation classes. Registry exchanges often carry these facts in flat records. The rows below name typical source records and fields and the FHIR R5 resource that carries each one. They are mapping decisions, not a general converter or claims of exact equivalence. Preserve the original source and explicitly record any unmapped field before replacing an adopter's source data.

| Source record or field | FHIR destination and conditions |
| --- | --- |
...
| `FacilityManagementAssignment` | A facility management assignment is ambiguous. First determine whether the source asserts service operation, premises upkeep, ownership, or institutional governance. Location.managingOrganization may carry a current upkeep assertion only when that role is established. It has no management period, so the source `valid_from`/`valid_to` are not transferred into that bare reference. Independently needed dated physical-asset roles use `AssetPartyRole` with explicit role meaning. |
| `FacilityAddressAssignment` | Native Location can retain an Address with its period of use. The cross-sector `AssetAddressAssignment` remains useful for school and agricultural premises history. Preserve physical/postal purpose. A separate arbitrary SpatialGeometry cannot be squeezed into `Location.position`, which is a point with latitude/longitude and optional altitude. Keep unsupported geometry through the cross-sector record and source links. |
...
Date boundaries require an explicit decision. A source `valid_to` often means the last applicable date. PublicSchema `end_date` is the last effective day, and FHIR [Period](https://hl7.org/fhir/R5/datatypes-definitions.html#Period) also includes its end, while allowing partial precision and times. Confirm the source boundary before converting; do not invent timestamps or copy a period onto another fact.
