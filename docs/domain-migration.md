...
JSON property keys do not acquire prefixes: a school still has `school_type`, while the generated JSON-LD context expands it to the scoped property URI. Unambiguous short class aliases, such as `Farm`, continue to be available in that context. The build fails if a published alias would disappear or expand to another URI, for example when a second domain adds a class with the same name; `schema/published_class_aliases.json` records the published aliases, and retiring one means removing it there and stating it in the release notes. Prefer the explicit canonical type `agri/Farm` or its absolute URI when inspecting migrations. An old absolute URI is not silently rewritten by a new context, and no `owl:sameAs` assertion is made for a retired shape.
...
The [relationship date guide and helper](relationship-date-migration.md) cover source records that carry `valid_from`/`valid_to` for relationships that use `start_date`/`end_date`. Under the helper's explicit inclusive-calendar-day source contract, an inclusive end of `2026-12-31` stays `2026-12-31`, because `end_date` is also the last effective day. Missing dates stay missing. Legal validity, registration and certification intervals keep their existing contracts. Namespace migration and date conversion are separate reviewed steps.
