...
Standards-body convergence supplements mapped-system convergence at the abstract-supertype level. Every abstract concept reports `system_count: 0/{total}` against the v1 mapped-system corpus because no mapped system exposes an explicit supertype by that name; convergence evidence accrues to the concrete subtypes instead. This convention is applied uniformly to the four core abstract supertypes (Agent, Event, Party, Profile), to intermediate abstract categorisers (Group, VitalEvent), and to Instrument. The abstractions themselves are aligned with widely-deployed standards (PROV-O for Agent and Event, FOAF/schema.org for Agent and Organization, FHIR QuestionnaireResponse for Profile, DPV for Party's consent framing). Convergence across those standards is acceptable evidence for introducing an abstract supertype, provided subtypes still carry mapped-system evidence of their own. See [ADR-008](../decisions/008-agent-organization.md) for this pattern applied to Agent and Organization.
...
A small number of **intermediate abstract categorisers** also carry `abstract: true` without being core supertypes. Group intermediates between Party and its concrete subtypes (Household, Family, Farm). VitalEvent intermediates between Event and the concrete vital events (Birth, Death, Marriage, MarriageTermination, etc.). Each intermediate groups siblings that share common structure but have no direct use of their own. They are abstract for the same reason as the core supertypes (never instantiated, always realised through a subtype) but they are not intended as cross-domain foundations.
