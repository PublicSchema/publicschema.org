...
**Status:** Draft design, partially implemented &middot; **Author:** PublicSchema working group &middot; **Implementation status reviewed:** 2026-09-07

## Current implementation

The repository provides the LinkML metric model in `schema/metrics.yaml`, draft catalog sources in `schema/metric_catalog/`, and a site catalog projection generated by `just build` (or `just metrics-data` for that projection alone). The checked-in catalogs currently contain 597 draft entries: 580 DHS entries and 17 ILO entries. Only three DHS entries have authored calculation blocks; the remaining entries are metadata seeds, not executable indicator definitions.

The build projects these sources into `dist/metrics_catalog.json` for browsing. It does not execute SQL, VTL, or CQL, establish calculation correctness, or supply a PublicSchema CQL ModelInfo binding. Metric-specific CSVW, SDMX, and RDF Data Cube publication are design targets, not implemented exports. The ordinary vocabulary RDF, JSON Schema, and download pipeline does not establish those metrics publication capabilities.

The sections below describe the proposed model and conformance requirements. Rules about executable calculations, default libraries, agency acceptance, and publication formats are design targets unless explicitly identified above as implemented. The worked example is illustrative; its library and schema URLs are not promised published artifacts. Roadmap versions and dates record the original proposal, not a release commitment.
...
**Derivable-only design target.** A complete metric definition must include a calculation against PS-mapped microdata. A catalog seed without that calculation does not yet meet this target. Sourced indicators (compiled from upstream NSI / agency tables with no microdata derivation) remain valuable but are out of scope here; they belong with the existing provenance layer.
...
The proposed metrics primitive aims to address all three: one **canonical executable definition** per indicator, **alignment URIs** that route a single observation into many consumer DSDs, and a **declarative computation block** that is auditable and re-runnable.
...
- Replacing statistical agency tooling. The proposed publication layer describes the metric and would emit SDMX; it does not run SDMX Web Services or agency registries.
...
- Defining a new computation runtime. The criteria language is pluggable (IANA-media-typed); proposed integrations target existing SQL, VTL, and CQL engines.
- AI / ML model evaluation metrics. Adjacent and worth a separate spec; not folded in here.

## Conformance tiers

The proposed conformance model defines three tiers. These are not certifications of the current catalog or build:
...
The conceptual model has seven main classes, with calculation and structured-value helpers in LinkML. The shape is the SDMX 3.0 information model (dimensions, measures, attributes, code lists, observations) extended with FHIR Measure's computation semantics. The [observation / scoring separation](/docs/design-principles/#6-observation-and-scoring-separation) (Principle 6) extends from per-record scoring to aggregate metrics: a `MetricObservation` is to a `Metric` what a `ScoringEvent` is to a `ScoringRule`. Metrics may carry [`core: true`](/docs/design-principles/#9-core-and-extended-property-tiers) (Principle 9) to mark the must-have subset.
...
- `class_uri` &mdash; `publicschema:Metric`. Proposed RDF mapping: one `qb:MeasureProperty` and one `skos:Concept`.
...
- `class_uri` &mdash; `publicschema:MetricDimension`; proposed RDF mapping: `qb:DimensionProperty`.
...
The proposed initial library of MetricDimensions includes: country (ISO 3166-1 alpha-2 bound to `CL_AREA`), region / admin1 (OCHA COD-AB), sex (PS sex vocab bound to `CL_SEX(2.1)`), age (single year + standard bands, bound to `CL_AGE`), urban / rural, wealth quintile, disability status (WG-SS), program type (PS program taxonomy), benefit modality, reference period, frequency (`FREQ`).

### `MetricAttribute`

A non-key annotation on an observation, series, or dataset. The proposed default library mirrors [SDMX Cross-Domain Concepts](https://sdmx.org/sdmx_cdcl/) to support a future SDMX emitter. Default identifiers and **correct attachment levels** (SDMX 3.0 IM):
...
The proposed foundation scope selects six PS-authored default attributes: `OBS_STATUS`, `UNIT_MEASURE`, `UNIT_MULT`, `DECIMALS`, `cell_count`, `confidentiality_status`. The remaining attributes are deferred until a concrete use case needs them.

### `MetricCalculation`

The declarative computation. See [Computation model](#computation-model). One per Metric; richer variants carried by a library reference. The per-record analog is [`ScoringRule`](/ScoringRule/) (`schema/misc.yaml`); `MetricCalculation` aggregates across subjects.
...
Bound to `qb:Observation` and SDMX 3.0 Observation. `MetricObservation` is `is_a: Event` ([Principle 5](/docs/design-principles/#5-abstract-supertypes)); it inherits `identifiers` from `Event`.

### `MetricReport`

A bundle of observations with publisher metadata: `publisher` (ranges over [`Agent`](/docs/design-principles/#5-abstract-supertypes)), `published_at`, `reference_date`, `methodology_uri`, `license`, `contact_uri`, `observations[]`, `dsd_uri`. Bound to `qb:DataSet`.

### `Period`

The temporal coverage of a MetricObservation. PS reifies the period (rather than carrying inline `start_date` / `end_date` like other `Event` subtypes) because the SDMX / Data Cube emitter needs a single URI-addressable interval to satisfy [W3C Data Cube integrity constraint IC-11](https://www.w3.org/TR/vocab-data-cube/#wf). Slots: `period_type` (`point_in_time` / `span` / `cohort` / `fiscal_year` / `calendar_year`), `start_date`, `end_date`, `granularity` (year / quarter / month / day), `reference_period_type`, `frequency` (`FREQ` codelist value: A / Q / M / D / W / H). `end_date` is the last covered day, as everywhere in PublicSchema ([ADR-027](../decisions/027-end-date-boundary.md)): calendar year 2024 runs from `2024-01-01` to `2024-12-31`, the same way a FHIR `Period` or a DCAT temporal coverage states it.
...
The proposed calculation validation must check `populations[*].type` against the metric's `scoring` value. The current catalog projector does not perform this semantic validation.

### Criteria language

Every `criteria` field is paired with a `language` IANA media type. PublicSchema does not pick a winner; it pins the media type so consumers know which engine to invoke. **Three languages are proposed:**
...
3. **`text/cql`** &mdash; HL7 Clinical Quality Language ([spec v1.5.3](https://cql.hl7.org/), CMS / HEDIS adoption). Recommended for metrics that intersect clinical or quality-of-care (UHC effective coverage, immunisation, MNCH). A proposed CQL **ModelInfo** XML would bind CQL to PS classes, targeting the **[HL7 reference engine `cqframework/clinical_quality_language`](https://github.com/cqframework/clinical_quality_language)** (Apache-2.0, v4.8.0).
...
LinkML is the source of truth. This table lists proposed metric-specific outputs and potential implementation foundations. Availability of a LinkML generator or the existing `rdf_export.py` does not mean the corresponding metric publisher has been integrated or verified in this repository.

| Output | Why | Status | Priority |
|--------|-----|--------|----------|
| **JSON Schema** (per Metric, MetricObservation, MetricReport) | Default consumer format; tabular submitters; OpenAPI integration. | Metric-specific publication pending; LinkML `gen-jsonschema` is a possible foundation | P0 |
| **JSON-LD context** | Existing PS pattern. | Metric-specific publication pending; LinkML `gen-jsonld-context` is a possible foundation | P0 |
| **SHACL shapes** | Validation; existing PS pattern. | Metric-specific publication pending; LinkML `gen-shacl` is a possible foundation | P0 |
...
| **Excel** (per Metric definition) | Definition workbook, matching PS's existing per-concept Excel downloads. | Metric-definition workbooks pending; existing pipeline exports vocabulary concepts | P2 |
...
There is no canonical RDF `sdmx:` namespace published by SDMX-TWG. The schema lists only `qb:` in `exact_mappings`, with SDMX URN references in prose. The proposed Data Cube TTL emitter would need to enforce: IC-1, IC-2, IC-3, IC-4, IC-5, IC-6, IC-11, IC-13, IC-14. IC-7 through IC-10 (slices) are N/A in v0.1; v0.2 may add slice generation if a consumer asks. IC-12 (no duplicate dimension tuples) would also need validation at emission time; this validation is not implemented by the catalog projector.

## Seed corpus

The original proposal targeted **~240 derivable indicators across WB / ILO / UN / UNICEF**, each with an executable `MetricCalculation` and relevant `aligns_with` URIs. The estimates below are a research backlog, not the current implemented corpus; see [Current implementation](#current-implementation).
...
**Original corpus estimate: ~240 indicators.** v0.2 extends to OECD IDD / SOCX-derivable and Eurostat EU-SILC. v0.3 broadens to humanitarian (HXL-shaped) and program-management (leakage, processing time) indicators that lack a single inter-agency custodian.
...
The proposal would keep a metric at `bibo:draft` until custodian review, with a later status transition to record that review. The current seeds are all draft; catalog inclusion does not imply agency endorsement. Proposed custodian routing by domain:
...
The intended result is to reuse one observation across agency, graph, and dashboard formats. This example does not demonstrate those integrations: emitters, calculation execution, target DSD bindings, and consumer validation remain necessary. Alignment URIs alone do not establish acceptance by ILOSTAT or ASPIRE.
...
The normative source is [`schema/metrics.yaml`](https://github.com/PublicSchema/publicschema.org/blob/main/schema/metrics.yaml), wired into the composite schema via the `metrics` entry in `schema/publicschema.yaml`. Eleven classes (`Metric`, `MetricDimension`, `MetricAttribute`, `MetricCalculation`, `PopulationCriterion`, `Stratifier`, `MetricObservation`, `MetricReport`, `Period`, plus structured-pair helpers `DimensionValue` and `AttributeValue`), the slots they reference, and the enums (`ValueType`, `ScoringMethod`, `ImprovementNotation`, `RateAggregation`, `MetricTopic`, `AttachmentLevel`, `PopulationType`, `PeriodType`, `Granularity`, `Frequency`). The `ObsStatus` codes are not republished; they resolve via the SDMX URN `urn:sdmx:org.sdmx.infomodel.codelist.Codelist=SDMX:CL_OBS_STATUS(2.3)`.

Two LinkML-keyword adjustments are worth flagging for readers:

- `MetricDimension.range` and `MetricAttribute.range` (the spec's prose names) are spelled `value_range` in the LinkML file to avoid shadowing the `range` keyword. The proposed metric-specific RDF mapping would use the intended `range`; the catalog projection does not perform that mapping.
- `MetricObservation.value` is spelled `metric_value` to avoid colliding with `value` slots defined elsewhere in the composite. The illustrative JSON Schema below uses `value`; do not assume the current general schema exporter performs this rename.

`MetricObservation` is `is_a: Event`. `MetricReport.publisher` ranges over `Agent` (see [Principle 5](/docs/design-principles/#5-abstract-supertypes)). `dimension_values` and `attribute_values` are modelled as ordered lists of structured pairs (`DimensionValue` and `AttributeValue`) rather than maps, so SDMX DSD round-trip is stable.

## JSON Schema (inline)

The proposal calls for `metric.schema.json`, `metric-observation.schema.json`, and `metric-report.schema.json` at stable publication URIs. These metric-specific hand-authored artifacts are not currently supplied. The schema below is indicative and is not a published contract; in particular, its attribute map differs from the structured-pair list in the current LinkML model.
...
1. **Hierarchy and category schemes for dimensions.** SDMX dimensions carry hierarchies (M49 for REF_AREA, age-band hierarchies for AGE). The initial design calls for flat codelists; v0.2 adds hierarchy when an emission consumer requires it.
2. **Content Constraints.** Agencies attach `ContentConstraint` artefacts saying "for this dataflow, REF_AREA is restricted to these codes". The initial design proposes unconstrained DSDs; v0.2 may add a `MetricConstraint` shape if agency emission requires it.
...
6. **Multi-measure metrics.** SDMX 3.0 allows multiple Measures per DSD; The initial PS design targets single-measure publication only. Add `MetricMeasure` class in v0.2 if a real use case arrives.
...
2. **Net-new generator burden.** LinkML upstream has `gen-jsonschema`, `gen-shacl`, `gen-jsonld-context`. It does **not** have `gen-csvw`, `gen-sdmx`, or `gen-datacube`. The proposal estimated ~1500 LOC of generators in `build/` for the foundation phase (CSVW, SDMX-CSV, SDMX-ML structural messages, RDF Data Cube TTL). The emission-targets table marks each as `existing` vs. `to-build`. Upstreaming `gen-csvw` to LinkML is in the v0.2 roadmap.
3. **CQL non-FHIR ModelInfo is rare in production.** CQL the *language* supports custom ModelInfo XML; CQL *engines* mostly ship with FHIR-only bindings. The HL7 reference engine [`cqframework/clinical_quality_language`](https://github.com/cqframework/clinical_quality_language) (Apache-2.0, v4.8.0) accepts custom ModelInfo. The proposal calls for `PublicSchema-ModelInfo.xml` against the HL7 reference engine's ModelInfo schema in v0.2; budget one week with a CQL specialist.
