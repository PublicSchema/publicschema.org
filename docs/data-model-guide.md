...
Washington Group functioning items (WG-SS, WG-ES, CFM) and socio-economic survey answers (dwelling type, WASH service, assets, ICT, income) are point-in-time records produced by a defined instrument. They are modeled as `Profile` records (`FunctioningProfile`, `SocioEconomicProfile`), not as columns on Person or Household. Humanitarian profile subtypes (`AnthropometricProfile`, `FoodSecurityProfile`, `DwellingDamageProfile`) live in a sibling schema outside PublicSchema and follow the same pattern, extending PublicSchema's abstract `Profile` as their supertype.

A `Profile` carries who was observed, when, with which instrument, under which administration mode, and by whom. Profiles may also carry outputs derived by applying the instrument's canonical scoring rule: for example, the WG-SS disability identifier on `FunctioningProfile` or a PMT score on `SocioEconomicProfile`. These canonical outputs live directly on the Profile because they are produced by a single, well-defined rule that is inseparable from the instrument itself. Person retains small current-state summary flags (`functioning_status`, `nutrition_status`) so operational systems can query targeting and reporting without re-deriving from every administration; Household similarly retains `food_security_level`. The summary flags are denormalizations for convenience, not classifications. The Profile remains the source of truth. See [ADR-011](../decisions/011-humanitarian-profile-extraction.md).

### Non-canonical scoring is a distinct act from instrument administration

Non-canonical scoring methodologies (PMT formulas, PPI, multidimensional poverty indices, composite indices like CARI, alternate thresholds, or researcher-defined cutoffs) are cataloged as `ScoringRule` records. Applying a non-canonical rule to one or more Profiles or inline data produces a `ScoringEvent`, which carries the raw score and band. Keeping non-canonical scoring separate from the Profile lets the same data be scored against multiple rules without re-collecting, while canonical outputs remain close to the observations that produced them. See [ADR-010](../decisions/010-profile-derived-outputs.md).
