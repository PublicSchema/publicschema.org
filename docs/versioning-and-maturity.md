...
**Maturity is per-element, not per-dependency-chain.** A property's or a vocabulary's maturity is decided by that element's own contract stability, not by the maturity of the concepts that reference it. A date field such as `enrollment_date` can be normative even when the `Enrollment` concept that uses it is still candidate: the property's contract is "a date in ISO 8601 format that marks when an enrollment began", which is stable regardless of how many more properties `Enrollment` might grow. Likewise, a controlled vocabulary such as `sp/grievance-type` can be normative while the `Grievance` concept is candidate, because the vocabulary's contract is the set of codes and their meanings, not the wider shape of the concept that uses it. What a normative property/vocabulary locks down is its own shape and semantics; what remains candidate at the concept level is the concept's overall property set and relationships. Breaking the concept's shape does not break the normative property: adding, removing, or renaming properties on `Grievance` does not invalidate the `grievance_type` URI, definition, or value set. If the property's own definition needs to change, that still triggers the property's maturity rules (ADR + new URI at normative).
