# DEPRECATED: superseded by build/linkml_rdf_export.py (gen-owl + gen-shacl + rdflib bridge).
...
This is the pre-LinkML "synthesize-from-build-result" pipeline. The
production build now uses ``build/linkml_rdf_export.py`` which delegates
to LinkML's stock generators. The legacy module is preserved (with a
shim at ``build/rdf_export.py``) so comparison tests can still run it.
"""
