...
@pytest.mark.skip(
    reason="Superseded by tests/test_external_term_resolution.py, which "
    "reads the LinkML composite at schema/publicschema.yaml via SchemaView "
    "and resolves cited CURIEs through OAK's local-TTL adapter. This legacy "
    "implementation walked bespoke schema/concepts/ etc. directories that no "
    "longer exist post-cutover; left here only as documentation."
)
class TestDPVNamespacePinning:
    """Historical pre-cutover DPV pinning test. See class-level skip marker."""

    def test_cited_dpv_uris_resolve(self):
        pass
