...
- `base-profiles.zip` is a local ZIP container for eleven unmodified upstream `*.profile.json` documents. Member bytes are preserved; ZIP packaging is local, so the container's own SHA-256 in the manifest identifies this repository's archive only. Compare the per-member SHA-256 values with the upstream release. These are the published base StructureDefinitions for Bundle and the ten resource kinds used by the example. Their snapshots supply reference target constraints, not a new PublicSchema medical model.
