...
`triggering_event` is reused across CRVS annotation concepts (referencing VitalEvent) and DwellingDamageProfile (referencing HazardEvent). Its type is widened from `concept:VitalEvent` to `concept:Event` to support both; the concept definition of each consumer constrains which Event subtype is expected. This is consistent with decision 5 above: the property's own definition ("the event that triggered this record") stays true across contexts; only the expected Event subtype differs, and that is a constraint of the consuming concept rather than a fact about the property.
